...
function evidence(activityId:string): AcademyEvidenceProfile {
  ...
  return {modalities:result?['K','V','R']:['K','V'],primaryModality:result?'R':'V',knowledgeExplanationRequired:true,virtualDemonstrationRequired:true,physicalExecutionRequired:false,measuredOrReviewedResultRequired:result,physicalCompetenceClaim:false,reviewerRequired:false,measurableAcceptanceCriteria:['Conserva autoridad, referencia geométrica, unidad y aplicabilidad.','Propaga los datos pendientes y los conflictos.','No convierte un modelo o una captura en evidencia física.'],evidenceArtifacts:result?['registro local estructurado','dossier revisable','límites']:['estado digital','explicación','datos pendientes']}
}
...
export const ACADEMY_STAGE_5_ACTIVITY_PRESENTATIONS: readonly AcademyStage5ActivityPresentation[] = ACADEMY_STAGE_5_CATALOG.filter((item):item is typeof item & {requiredActivityId:string} => Boolean(item.requiredActivityId)).map((item) => ({
  ...
  instructions:['Formula la pregunta de interfaz o requisito.','Registra ambos componentes y la autoridad de cada dato.','Calcula solo con entradas completas y referencias geométricas compatibles.','Conserva los datos pendientes, los conflictos y la validación física pendiente.'],
