...
const tm = (): AcademySourceLocator => ({ sourceId: 'source.official.tm9-1575.inspection', documentLocator: 'reference-library/originals/TM 9-1575.pdf', page: 'impresas 20–22', section: 'III', verificationMethod: 'visual-pdf-inspection', verifiedAt: '2026-08-16' })
...
const specs: readonly Spec[] = [
  ...
  { visualDesignId: 'visual.stage3.measurement-terms.v1', questionId: 'q06-terms', question: '¿Qué diferencia resolución, precisión, exactitud e incertidumbre?', title: 'Cuatro conceptos no equivalentes', lessonIds: ['lesson.metrology.units-scale-resolution', 'lesson.metrology.precision-accuracy-uncertainty'], nodes: [['resolution','Resolución','incremento mostrado'],['precision','Precisión','proximidad repetida'],['accuracy','Exactitud','relación con referencia'],['uncertainty','Incertidumbre','duda asociada']], edges: [['resolution','precision','no implica'],['precision','accuracy','no implica'],['accuracy','uncertainty','se interpreta con']], source: vim('§§2.13, 2.15, 2.26 y 4.14'), limitation: 'No asigna valores a instrumentos o resultados.' },
...
export const ACADEMY_STAGE_3_VISUAL_DESIGNS: readonly (AcademyStage3VisualDesign & { questionId: string })[] = specs.map((spec) => {
  ...
  return {
    ...
    longDescription: `${spec.title}. ${spec.edges.map(([from, to, label]) => `${from} conduce a ${to}: ${label}`).join('. ')}.`,
