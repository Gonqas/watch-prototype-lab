...
function sectionsFor(item: AcademyStage4CatalogEntry): readonly AcademyStage4SectionSpec[] {
  ...
  const additionalVisuals = stage4VisualIds.slice(1).map((visualDesignId) => section(academyStage4SectionId.visual(item.lessonId,visualDesignId), 'Comparación documental complementaria', 'comparison', academyStage4VisibleSpanish('Compara ambos snapshots por hash, localizador, terminología, referencia y número de piezas. Un cambio de nombre o maquetación no equivale a una pieza nueva.'), 'after-first-substantive-source', { visualDesignId }))
