...
export function MetrologySurface() {
  ...
  const createMetadataBackup = async () => {
    ...
    try {
      if (isNativeApp()) {
        ...
        setStatus(`Backup de metadatos creado; ${result.omittedObjectCount} objetos declarados y no incluidos.`)
      } else {
        ...
        setStatus(`Backup de metadatos exportado; ${backup.objects.length} objetos binarios declarados y no incluidos.`)
      }
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo crear el backup.') }
  }
  ...
  const createFullBackup = async () => {
    ...
    try {
      ...
      setLastBackup(result); setRestorePreview(undefined)
      setStatus(`Backup completo verificado: ${result.objectCount - result.omittedObjectCount}/${result.objectCount} objetos.`)
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo crear el backup completo.') }
  }
  ...
  const confirmLastRestore = async () => {
    ...
    setError(undefined)
    try { await restoreNativeMetrologyBackup(restorePreview.backupId, restorePreview.manifestHash); if (repository) await refresh(repository); setRestorePreview(undefined); setStatus('Backup restaurado con copia de seguridad previa y hashes verificados.') }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo restaurar el backup.') }
  }
  ...
  return (
    <div className="academy-page metrology-page">
      ...
      <details className="metrology-data-safety"><summary>Protección, backup y restauración</summary><div><p>El backup de metadatos informa expresamente de las fotos omitidas. En Desktop, el backup completo incluye SQLite, manifiesto y objetos SHA-256.</p><div className="metrology-actions"><button type="button" onClick={() => void createMetadataBackup()}><Save size={15} />Backup de metadatos</button>{isNativeApp() && <button type="button" onClick={() => void createFullBackup()}><ShieldCheck size={15} />Backup completo</button>}{lastBackup && <button type="button" onClick={() => void previewLastRestore()}><RefreshCcw size={15} />Previsualizar restauración</button>}{restorePreview && <button type="button" disabled={restorePreview.missingOrCorruptObjectIds.length > 0 || restorePreview.conflictingObjectIds.length > 0} onClick={() => void confirmLastRestore()}><ClipboardCheck size={15} />Confirmar restauración</button>}</div>{lastBackup && <small>{lastBackup.backup.id} · {lastBackup.objectsIncluded ? 'con objetos' : 'solo metadatos'} · {lastBackup.manifestHash}</small>}{restorePreview && <p role="status">Restaurables: {restorePreview.restorableObjectIds.length}. Ausentes o corruptos: {restorePreview.missingOrCorruptObjectIds.length}. Conflictos: {restorePreview.conflictingObjectIds.length}. No se sobrescribirá ningún objeto distinto.</p>}</div></details>
      ...
      {tab === 'inspection' && <section className="metrology-split"><div><InspectionPanel repository={repository} profileId={profileId} specimen={selectedSpecimen} sessions={data.sessions} observations={data.observations} findings={data.findings} run={run} /></div><div><div className="metrology-image-picker">{specimenImages.map((image) => <button type="button" className={selectedImage?.id === image.id ? 'is-active' : undefined} key={image.id} onClick={() => setSelectedImageId(image.id)}>{image.id.slice(-10)}</button>)}</div><MetrologyImageWorkbench image={selectedImage} original={selectedOriginal} thumbnail={selectedThumbnail} calibration={selectedCalibration} annotations={selectedAnnotations} onCreateCalibration={createCalibration} onCreateAnnotation={createAnnotation} /></div></section>}
...
function InspectionPanel({ repository, profileId, specimen, sessions, observations, findings, run }: { repository?: HorologyMetrologyRepository; profileId: string; specimen?: PhysicalSpecimen; sessions: InspectionSession[]; observations: InspectionObservation[]; findings: InspectionFinding[]; run: (operation: () => Promise<void>, success: string) => Promise<void> }) {
  ...
  return <section className="metrology-inspection-card"><span className="academy-kicker">SESIÓN DE INSPECCIÓN</span><h2>{specimen?.displayName ?? 'Selecciona una unidad'}</h2>{!activeSession ? <button type="button" disabled={!specimen} onClick={createSession}><Plus size={15} />Iniciar sesión</button> : <><p>Sesión activa desde {activeSession.startedAt}. Todas las operaciones visuales pueden restaurarse.</p><form onSubmit={addObservation} className="metrology-form"><label>Qué observas<textarea value={observationText} onChange={(event) => setObservationText(event.target.value)} placeholder="Describe solo lo visible, sin diagnosticar." /></label><button type="submit"><Save size={14} />Guardar observación</button></form><form onSubmit={addFinding} className="metrology-form"><label>Hallazgo asociado<textarea value={findingText} onChange={(event) => setFindingText(event.target.value)} placeholder="Clasifica lo visible; el sistema no emitirá un diagnóstico." /></label><div className="metrology-form-grid"><label>Categoría<select value={findingCategory} onChange={(event) => changeCategory(event.target.value as FindingCategory)}>{Object.keys(FINDING_TYPES_BY_CATEGORY).map((value) => <option value={value} key={value}>{value}</option>)}</select></label><label>Tipo<select value={findingType} onChange={(event) => setFindingType(event.target.value as FindingType)}>{FINDING_TYPES_BY_CATEGORY[findingCategory].map((value) => <option value={value} key={value}>{value}</option>)}</select></label><label>Gravedad observacional<select value={findingSeverity} onChange={(event) => setFindingSeverity(event.target.value as FindingSeverity)}><option value="note">nota</option><option value="minor">menor</option><option value="significant">significativo</option><option value="critical-unknown">crítico desconocido</option></select></label><label>Confianza<select value={findingConfidence} onChange={(event) => setFindingConfidence(event.target.value as InspectionFinding['confidence'])}><option value="high">alta</option><option value="medium">media</option><option value="low">baja</option><option value="unknown">desconocida</option></select></label></div><label>Hipótesis opcional<textarea value={hypothesis} onChange={(event) => setHypothesis(event.target.value)} placeholder="Se guarda separada y con confianza baja; no es un diagnóstico." /></label><button type="submit" disabled={!observations.some(({ sessionId }) => sessionId === activeSession.id)}><ClipboardCheck size={14} />Guardar hallazgo</button></form><div className="metrology-timeline">{observations.filter(({ sessionId }) => sessionId === activeSession.id).map((observation) => <article key={observation.id}><strong>Observación</strong><p>{observation.description}</p></article>)}{findings.filter(({ sessionId }) => sessionId === activeSession.id).map((finding) => <article key={finding.id}><strong>{finding.category} · {finding.type} · {finding.severity}</strong><p>{finding.finding}</p>{finding.hypothesis && <small>Hipótesis ({finding.hypothesisConfidence}): {finding.hypothesis}</small>}</article>)}</div></>}</section>
}
